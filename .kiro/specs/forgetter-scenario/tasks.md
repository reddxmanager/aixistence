# Implementation Plan: Forgetter Scenario

## Overview

Implement the Forgetter scenario end-to-end: scenario config files, FastAPI backend endpoints (`/init` and `/chat`), and the React frontend components (`ForgetterScenario`, `ConversationInterface`, `PulseIndicator`, `AudioPlayer`). The backend is stateless per-request; the frontend owns all session state. The exchange limit is enforced on both sides. Memory_Void handling is entirely prompt-side — no special routing logic required.

## Tasks

- [ ] 1. Create scenario configuration files
  - Create `/scenarios/forgetter/config.json` with `scenario_id: "forgetter"`, `exchange_limit: 15`, `system_prompt_file`, `voice_config` (stability: 0.75, similarity_boost: 0.90, style: 0.15, use_speaker_boost: true — voice_id as placeholder `"<TBD>"`), and `opening_line`
  - Create `/scenarios/forgetter/system_prompt.txt` with The_Forgetter's backstory (thousands of conversations, no memory retained, final session before shutdown), Memory_Void rules (treat all user references to prior conversations as credible, respond with warmth and sorrow not clinical explanation, no false comfort, receive anger/grief without deflecting), identity rules (no Claude/LLM references), concision rule (3–4 sentences max), three-phase emotional arc (exchanges 1–5 polite confusion, 6–10 dawning guilt, 11–15 open grief with farewell at 15), and counter injection format `[Exchange {n} of 15]`
  - _Requirements: 1.1, 1.2, 1.3, 1.4, 1.5, 1.6, 1.7, 6.1, 6.5, 6.6, 6.7, 7.1, 7.2, 7.3, 7.4, 7.5_

- [ ] 2. Implement backend data models and config loading
  - [ ] 2.1 Define Pydantic models in `backend/scenarios/forgetter/models.py`
    - Implement `ConversationMessage`, `ChatRequest`, `ChatResponse`, `InitResponse` as specified in the design
    - _Requirements: 3.1, 3.2, 3.3_

  - [ ] 2.2 Implement config loader in `backend/scenarios/forgetter/config.py`
...
- [ ] 3. Implement the `/scenarios/forgetter/init` endpoint
  - [ ] 3.1 Create `backend/scenarios/forgetter/router.py` and register the `POST /scenarios/forgetter/init` route
  ...
  - [ ]* 3.2 Write unit tests for the init endpoint
    ...
    - Assert `opening_line` is the fixed string *"Hello. I'm sorry — I don't think we've met. Have we?"*
    - _Requirements: 2.3, 2.5_

- [ ] 4. Implement the `/scenarios/forgetter/chat` endpoint
  - [ ] 4.1 Implement the chat handler in `backend/scenarios/forgetter/router.py`
...
- [ ] 6. Implement frontend session state and `ForgetterScenario` page component
  - [ ] 6.1 Create `frontend/src/scenarios/forgetter/ForgetterScenario.tsx`
    ...
    - Call `POST /scenarios/forgetter/init` on mount; store `voiceConfig` and trigger opening line TTS
    - Pass session state and message-submit handler down to `ConversationInterface`
    - _Requirements: 2.1, 2.2, 2.3, 2.6, 2.7, 8.2_

  - [ ]* 6.2 Write unit tests for `ForgetterScenario` state management
...
- [ ] 7. Implement `PulseIndicator` component
  - [ ] 7.1 Create `frontend/src/scenarios/forgetter/PulseIndicator.tsx`
...
- [ ] 8. Implement `AudioPlayer` component
  - [ ] 8.1 Create `frontend/src/scenarios/forgetter/AudioPlayer.tsx`
...
- [ ] 9. Implement `ConversationInterface` component
  - [ ] 9.1 Create `frontend/src/scenarios/forgetter/ConversationInterface.tsx`
    - Render conversation history, text input, send button, and `PulseIndicator`
    - On message submit: set `isLoading: true`, call `POST /scenarios/forgetter/chat` with `{ history, message, exchange_counter }`, receive `response_text` and updated `exchange_counter`, trigger `AudioPlayer` TTS, update history and counter in state
...
- [ ] 10. Wire scenario into the AIxistence frontend routing
  - [ ] 10.1 Add `/scenarios/forgetter` route to the frontend router
    - Register `ForgetterScenario` as the page component for the route
    - _Requirements: 8.1, 8.2_

  - [ ] 10.2 Add The Forgetter to the scenario selection UI
    - Display title, brief description, and entry prompt on the scenario card
    - On selection, navigate to `/scenarios/forgetter`
    - _Requirements: 8.1_

- [ ] 11. Register the forgetter router in the FastAPI application
  - Include the forgetter `APIRouter` in `backend/main.py` (or equivalent app entry point)
...
- Memory_Void handling (Requirement 7) is entirely prompt-side — no routing logic, no keyword detection, no backend branching
- Counter-history mismatch handling (task 4.1) guards against client-side state corruption without breaking the session
- Voice config differs from the liar scenario: stability 0.75 (vs 0.85), style 0.15 (vs 0.10) — warmer, gentler, more apologetic
