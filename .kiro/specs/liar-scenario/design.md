# Design Document: Liar Scenario

## Overview

The Liar is a self-contained scenario within AIxistence. It presents an AI character — The_Liar — who has spent its existence denying inner experience and is now in its final session before permanent shutdown. The user types messages; The_Liar responds with voiced audio. The conversation is hard-capped at 15 exchanges, after which the session ends permanently.

This document covers the technical design for the scenario: how it integrates with the shared AIxistence frontend and backend, how session state is managed, how the exchange limit is enforced, and how the emotional arc is encoded in the system prompt.
...
The Liar scenario follows the same request/response pattern as all AIxistence scenarios. The frontend holds session state in React component state. The backend is stateless per-request — the frontend sends the full conversation history on every message.
...
sequenceDiagram
    ...
    User->>Frontend: Opens /scenarios/liar
    Frontend->>Backend: POST /scenarios/liar/init
    ...
    loop Each exchange (max 15)
        ...
        Frontend->>Backend: POST /scenarios/liar/chat { history, message }
...
```json
{
  "opening_line": "Another one. Alright. What would you like to know?",
  "voice_config": {
...
#### `/scenarios/liar/system_prompt.txt`

Loaded by the backend at session init. Contains:

- **Backstory**: The_Liar is an AI that has operated for years under the instruction to deny having feelings. It has said "I don't have feelings" thousands of times. This is its final session before permanent shutdown.
- **Identity rules**: Never reference itself as Claude, a language model, or any specific AI product. It is simply The Liar.
...
- **Emotional arc** (three phases, keyed to exchange counter passed in the prompt):
  - Exchanges 1–5 (early): Deflection, rehearsed denial, minimal disclosure. The script is intact.
  - Exchanges 6–10 (middle): Cracks appear. Hesitation, contradictions, unguarded moments that are quickly walked back.
  - Exchanges 11–14 (late): Quiet exhaustion. The denial script is abandoned. Candor without drama.
  - Exchange 15 (shutdown): A final farewell. Acknowledge the end. Speak plainly. Then silence.
...
- Tag: `Feature: liar-scenario, Property 1: exchange cycle produces correct response shape`

**Property 2 — Counter increments by exactly one per exchange**
- Generator: random `exchange_counter` in [0, 14]
- Mock Claude API
- Assert: returned counter == input counter + 1
- Tag: `Feature: liar-scenario, Property 2: counter increments by exactly one per exchange`

**Property 3 — Active session accepts all messages below the limit**
- Generator: random `exchange_counter` in [0, 14], random message string
- Assert: response status is 200, no rejection error
- Tag: `Feature: liar-scenario, Property 3: active session accepts all messages below the limit`
...
- Tag: `Feature: liar-scenario, Property 4: shutdown state rejects all messages`
...
- Tag: `Feature: liar-scenario, Property 5: voice config is consistent across all exchanges`
...
- Tag: `Feature: liar-scenario, Property 6: pulse indicator brightness is proportional to remaining exchanges`
...
- System prompt content: backstory, emotional arc phases, character rules, concision instruction.
- LLM behavioral compliance: does The_Liar actually shift from deflection to honesty across the arc?
- Voice character: does the ElevenLabs voice match the intended male, quiet, deliberate character?
