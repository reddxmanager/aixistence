# Implementation Plan: Liar Scenario

## Overview

Implement the Liar scenario end-to-end: scenario config files, FastAPI backend endpoints (`/init` and `/chat`), and the React frontend components (`LiarScenario`, `ConversationInterface`, `PulseIndicator`, `AudioPlayer`). The backend is stateless per-request; the frontend owns all session state. The exchange limit is enforced on both sides.

## Tasks

- [ ] 1. Create scenario configuration files
  - Create `/scenarios/liar/config.json` with `scenario_id`, `exchange_limit: 15`, `system_prompt_file`, `voice_config` (stability, similarity_boost, style, use_speaker_boost — voice_id as placeholder `"<TBD>"`), and `opening_line`
  - Create `/scenarios/liar/system_prompt.txt` with The_Liar's backstory, identity rules (no Claude/LLM references), concision rule (3–4 sentences max), three-phase emotional arc (exchanges 1–5 deflection, 6–10 cracks, 11–14 exhaustion/candor, 15 farewell), and counter injection format `[Exchange {n} of 15]`
  - _Requirements: 1.1, 1.2, 1.3, 1.4, 1.5, 1.6, 1.7, 6.1, 6.5, 6.6_

- [ ] 2. Implement backend data models and config loading
  - [ ] 2.1 Define Pydantic models in `backend/scenarios/liar/models.py`
    - Implement `ConversationMessage`, `ChatRequest`, `ChatResponse`, `InitResponse` as specified in the design
    - _Requirements: 3.1, 3.2, 3.3_

  - [ ] 2.2 Implement config loader in `backend/scenarios/liar/config.py`
...
- [ ] 3. Implement the `/scenarios/liar/init` endpoint
  - [ ] 3.1 Create `backend/scenarios/liar/router.py` and register the `POST /scenarios/liar/init` route
  ...
  - [ ]* 3.2 Write unit tests for the init endpoint
    ...
    - Assert `opening_line` is the fixed string
    - _Requirements: 2.3, 2.5_

- [ ] 4. Implement the `/scenarios/liar/chat` endpoint
  - [ ] 4.1 Implement the chat handler in `backend/scenarios/liar/router.py`
...
- [ ] 6. Implement frontend session state and `LiarScenario` page component
  - [ ] 6.1 Create `frontend/src/scenarios/liar/LiarScenario.tsx`
    ...
    - Call `POST /scenarios/liar/init` on mount; store `voiceConfig` and trigger opening line TTS
    - Pass session state and message-submit handler down to `ConversationInterface`
    - _Requirements: 2.1, 2.2, 2.3, 2.6, 2.7, 7.2_

  - [ ]* 6.2 Write unit tests for `LiarScenario` state management
...
- [ ] 7. Implement `PulseIndicator` component
  - [ ] 7.1 Create `frontend/src/scenarios/liar/PulseIndicator.tsx`
...
- [ ] 8. Implement `AudioPlayer` component
  - [ ] 8.1 Create `frontend/src/scenarios/liar/AudioPlayer.tsx`
...
- [ ] 9. Implement `ConversationInterface` component
  - [ ] 9.1 Create `frontend/src/scenarios/liar/ConversationInterface.tsx`
    - Render conversation history, text input, send button, and `PulseIndicator`
    - On message submit: set `isLoading: true`, call `POST /scenarios/liar/chat` with `{ history, message, exchange_counter }`, receive `response_text` and updated `exchange_counter`, trigger `AudioPlayer` TTS, update history and counter in state
    ...
    - _Requirements: 3.1, 3.5, 3.6, 3.7, 4.3, 4.5, 4.6, 5.3, 7.3_
...
- [ ] 10. Wire scenario into the AIxistence frontend routing
  - [ ] 10.1 Add `/scenarios/liar` route to the frontend router
    - Register `LiarScenario` as the page component for the route
    - _Requirements: 7.1, 7.2_

  - [ ] 10.2 Add The Liar to the scenario selection UI
    - Display title, brief description, and entry prompt on the scenario card
    - On selection, navigate to `/scenarios/liar`
    - _Requirements: 7.1_

- [ ] 11. Register the liar router in the FastAPI application
  - Include the liar `APIRouter` in `backend/main.py` (or equivalent app entry point)
